...
public interface PreviousAgentReportWTDRepository extends JpaRepository<AgentReportWTDModel, Long>{
	@Query(value="SELECT\r\n" + 
			"c.eid as EID,\r\n" + 
			...
			"WHERE (c.eid = ?1 AND (DATE(c.createddate)>= DATE(?2) AND DATE(c.createddate) <= DATE(?3)) )", nativeQuery = true)
	AgentReportWTDModel getWtdData(String agentPhoneId, String fromDate,String toDate);
