...
public interface PreviousSaasAgentReportWTDRepository extends JpaRepository<SAASAgentReportWTDModel, String>{
	...
	@Query(value="SELECT\r\n"+
			...
			"WHERE (c.agent_login = ?1 AND (DATE(c.createddate)>= DATE(?2) AND DATE(c.createddate) <= DATE(?3)) )", nativeQuery = true)
	SAASAgentReportWTDModel getWTDSaasReport(String agentLogin, String fromDate,String toDate);
